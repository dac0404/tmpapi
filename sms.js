...
function insertsms(wIdContact, wsmsTxtSend, wsmsTxtReceived, wFrom, wTo, smsUrlMms){
    var query ="call smsInsert(?,?,?,?,?,?)";
    conectionDB();

    cnn.query(query,[wIdContact, wsmsTxtSend, wsmsTxtReceived, wFrom, wTo, smsUrlMms], function(err, result, fields){
        if(err){
            console.log(err);
            cnn.end();
            return;
